...
export class HttpClient {
    ...
    while (true) {
      ...
      try {
        ...
        if (this.retryPolicy.shouldRetry(response.status, retryCount)) {
          this.logger.warn(`Request failed with status ${response.status}. Retrying (${retryCount + 1})...`);
          await this.retryPolicy.wait(retryCount);
          retryCount++;
          continue;
        }

        throw this.createError(response.status, errorBody, url, options.method || 'GET');
      } catch (error: any) {
        clearTimeout(timeoutId);
        ...
        if (error.name === 'AbortError') {
          throw new TimeoutError(`Request timeout after ${options.timeoutMs || this.defaultTimeoutMs}ms`);
        }
  ...
  private buildUrl(urlPath: string, query?: Record<string, any>): string {
    ...
    const baseUrl = isAbsolute ? urlPath : `${this.defaultBaseUrl}${urlPath.startsWith('/') ? urlPath : '/' + urlPath}`;
  ...
  private createError(status: number, responseBody: any, url: string, method: string): GBPApiError {
    const message = responseBody?.error?.message || `API request failed with status ${status}`;
    const code = responseBody?.error?.status || responseBody?.error?.code;

    if (status === 401) {
      return new AuthenticationError(message, { code, url, method, responseBody });
    }
    if (status === 429) {
      return new RateLimitError(message, { code, url, method, responseBody });
    }

    return new GBPApiError(message, status, { code, url, method, responseBody });
  }
}
